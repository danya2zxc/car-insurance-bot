...
from app.utils.file_utils import download_user_photo, get_passport_extracted_text, get_vehicle_extracted_text
...
class PhotoProcessor:
    ...
    async def process_photo(message: Message, state: FSMContext, bot: Bot, doc_type :str):
        ...
        if msg_to_edit_id:
            try:
                await bot.edit_message_reply_markup(chat_id=message.chat.id, message_id=msg_to_edit_id, reply_markup=None)
        ...
        try:
            if doc_type == "passport":
                ...
                if not mindee_data:
                    await processing_msg.edit_text("🛑 Passport photo is not valid. Please send a clear photo of your passport.")
